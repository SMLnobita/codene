/*
============================================================
 DE 5 (2026): SINH VIEN (ho ten, MSSV, ngay sinh, 5 diem)
   1. Doc danh sach sinh vien tu input.txt
   2. SHELL SORT xep TANG theo ngay/thang/nam sinh -> ghi output.txt
...
/* ============ 2. SHELL SORT theo ngay sinh (tang dan) ============ */
void ShellSortNgay(SINHVIEN a[], int n) {
    for (int h = n / 2; h > 0; h /= 2) {
        for (int i = h; i < n; i++) {
            SINHVIEN x = a[i];
            int j = i - h;
            while (j >= 0 && KeyNgay(a[j]) > KeyNgay(x)) {
                a[j + h] = a[j];
                j -= h;
            }
            a[j + h] = x;
...
int main() {
    ...
    SapXepTheoMSSV(a, n);
    const char *timMSSV = "2320000010";
    int vt = TimNhiPhanMSSV(a, n, timMSSV);
    if (vt != -1) {
        printf("\nTim nhi phan thay %s -> cap nhat ngay sinh thanh 30/12/2005\n", timMSSV);
        a[vt].ns.ngay = 30; a[vt].ns.thang = 12; a[vt].ns.nam = 2005;
    } else printf("\nKhong tim thay %s\n", timMSSV);

    ShellSortNgay(a, n);
    printf("\nSAU SHELL SORT theo ngay sinh (tang dan):\n"); InDS(a, n);
