/*
============================================================
 DE 14: CAY NHI PHAN AVL bang danh sach lien ket
   1. Cai dat cay AVL.
 ...
 Ghi chu: AVL la BST ma tai moi node, do lech chieu cao
 cua cay con trai va phai khong qua 1.
============================================================
*/
#include <cstdio>
#include <cstdlib>
...
// Do lech (balance factor) = h(trai) - h(phai)
...
NODE* CanBang(NODE *p) {
    ...
    if (bf > 1) {                          // lech TRAI
        ...
        return QuayPhai(p);                // LL hoac LR
    }
    if (bf < -1) {                         // lech PHAI
        if (BalanceFactor(p->right) > 0)   // truong hop RL
            p->right = QuayPhai(p->right);
        return QuayTrai(p);                // RR hoac RL
    }
    return p;                              // da can bang
}
...
NODE* InsertNode(NODE *t, int x) {
    ...
    return CanBang(t);                     // can bang lai tren duong di len
}
...
// ---- Tao ngau nhien mot mang chu so roi dung de dung AVL ----
...
// In cay theo muc, kem do lech de thay tinh can bang
