/*
============================================================
 DE 3: KIEU DU LIEU SINH VIEN
   - Ho ten: toi da 20 ki tu | MSSV: 10 ki tu SO | 5 diem so thuc
 Yeu cau:
   1. Nhap, tim theo MSSV, cap nhat ho ten.
   2. RADIX SORT de xep TANG ma sinh vien.
      (MSSV gom 10 ki tu so -> radix theo tung chu so tu phai sang trai)
============================================================
*/
#include <cstdio>
#include <cstring>
...
/* ================= RADIX SORT theo MSSV (chuoi 10 chu so) =================
   Xu ly tung vi tri ki tu tu PHAI sang TRAI (LSD radix sort).
   Moi lo ung voi mot chu so '0'..'9' -> on dinh -> tang dan. */
void RadixSort(SINHVIEN a[], int n) {
    SINHVIEN *tmp = new SINHVIEN[n];
    int count[10];
    for (int pos = LEN_MSSV - 1; pos >= 0; pos--) {   // tu chu so cuoi len
        for (int d = 0; d < 10; d++) count[d] = 0;
        for (int i = 0; i < n; i++)                   // dem so luong moi lo
            count[a[i].mssv[pos] - '0']++;
        for (int d = 1; d < 10; d++)                  // cong don vi tri
            count[d] += count[d - 1];
